      ...
      if (sub.linked_license_id && licenseStatus && paymentStatus) {
        ...
        if (linkedLicense && linkedLicense.is_legacy !== true) {
          ...
          if (licenseStatus === "active" && paymentStatus === "paid") {
            ...
            if (extensionDays > 0) {
              const currentExpiry = linkedLicense.expires_at ? new Date(linkedLicense.expires_at) : null;
...
app.post("/admin/licenses/reset-hwid", requireAdmin, async (req, res) => {
  try {
    ...
    const { error } = await supabase
      .from("licenses")
      .update({
        hwid: null, gpu_name: null, gpu_driver_version: null,
        gpu_raw_driver_version: null, gpu_is_laptop: null,
        gpu_brand: null, cpu_brand: null,
        last_gpu_reported_at: null, suggested_driver_status: null,
        suggested_driver_latest: null, suggested_driver_download_url: null,
        suggested_driver_checked_at: null, approved_driver_latest: null,
        approved_driver_download_url: null, driver_note: null,
        updated_at: new Date().toISOString()
      })
...
app.post("/admin/subscriptions/create", requireAdmin, async (req, res) => {
  try {
    ...
    const { error: linkError } = await supabase
      .from("licenses")
      .update({ linked_subscription_ref: subscriptionRef, payment_required: true, payment_status: "pending", updated_at: nowIso })
      .eq("license_key", license_key);
    ...
    try {
      ...
      if (resolved.licenseStatus && resolved.paymentStatus) {
        await supabase.from("licenses")
          .update({ status: resolved.licenseStatus, payment_status: resolved.paymentStatus, updated_at: nowIso })
          .eq("license_key", license_key);
      }
    } catch (syncErr) {
